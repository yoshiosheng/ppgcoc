...
// Custom JavaScript Function To Make Use Of Search Auto Complete
function enable_search_ahead(){
	...
	$("#nav-search-input").typeahead({
		...
		source: function (query, process) {
			var arrAccs = [];
			return $.getJSON('../search/search_autocomplete', {query: query}, function(data) {
				for(var i=0, X=data.length; i<X; i++) {
					mapAccs[data[i].baname] = data[i].baid;
					arrAccs.push(data[i].baname);
				}
				return process(arrAccs);
	        });
	    },
		updater: function(item) {
			$("#nav-search-input").focus();
			$("#nav-search-input").select();
			window.location = "../account/accounts?accountId=" + encodeURIComponent(mapAccs[item]);
			return item;


		},
		matcher: function(item) {
			return ~item.toLowerCase().lastIndexOf(this.query.toLowerCase(), 0);
	    },
	    items: 10
	// Manually Auto Focus Search Textbox To Fix The Compatibility Issue Of Typeahead With HTML5's "autofocus" Attribute 
	// Manually Auto Select Search Text
	}).focus().select();
}

// Disable Search Auto Complete First Item Selection
$.fn.typeahead.Constructor.prototype.render = function(items) {
	var name = this;
	items = $(items).map(function (i, item) {
		i = $(name.options.item).attr('data-value', item);
		i.find('a').html(name.highlighter(item));
		return i[0];
	});
...
// Manually Overriding Auto Complete List Behaviour And Search Form Submission During None Search Auto Complete Item Selection
$.fn.typeahead.Constructor.prototype.select = function() {
	var selection = this.$menu.find('.active');
	if(selection.length) {
		var val = selection.attr('data-value');
		this.$element
			.val(this.updater(val))
	     	.change();
		return this.hide();
	}
	return document.getElementsByClassName(this.$element[0].form.className)[0].submit();
};
